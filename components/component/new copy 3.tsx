...
const NewChapter: FC<NewChapterProps> = ({ notebookId }): ReactNode => {
  ...
  const [questionCounts, setQuestionCounts] = useState<{ comprehensive_coverage: number; important_details: number; key_concepts: number }>({ comprehensive_coverage: 0, important_details: 0, key_concepts: 0 });
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (pdfFiles.length > 0) {
        ...
        response = await axios.post('http://localhost:5000/process_notes', formData, {
      ...
      const { comprehensive_coverage, important_details, key_concepts, ocr_content } = response.data;
      setQuestionCounts({ comprehensive_coverage, important_details, key_concepts });
      setOcrContent(ocr_content);
      setNumberOfQuestions(comprehensive_coverage + important_details + key_concepts);
      setLoading(false);
      ...
      {!showQuestionStyles ? (
        ...
        <form onSubmit={handleQuestionStylesSubmit} className="flex flex-col items-center w-full space-y-4">
          ...
          <div className="flex flex-wrap justify-center space-x-2">
            {['Overview', 'General', 'Very Comprehensive'].map((type, index) => (
              <QuestionTypeButton
                key={type}
                type={type}
